...
本地事务，也就是传统的**单机事务**。在传统数据库事务中，必须要满足四个原则： 原子性（atomicity，或称不可分割性）、一致性（consistency）、隔离性（isolation，又称独立性）、持久性（durability）
...
![image-20211203152732994](assets/image-20211203152732994.png)



2）修改application.yml文件，将事务模式修改为AT模式即可：（**每个关联事务的微服务中**）
...
注意：把该表导入到seata-demo数据库中

![image-20211203155248081](assets/image-20211203155248081.png)
    ...
    public boolean cancel(BusinessActionContext context) {
      ...
      //进行判断空回滚
        if (accountFreeze == null) {
            //对account_freeze表进行插入数据
            accountFreeze=new AccountFreeze();
            accountFreeze.setState(AccountFreeze.State.CANCEL);
            accountFreeze.setFreezeMoney(0); //金额设置0
            //获取用户id
        ...
        //2.修改account_freeze表，冻结金额为0，state为2
        accountFreeze.setState(AccountFreeze.State.CANCEL);
...
public class AccountTCCServiceImpl implements AccountTCCService {
    ...
    public boolean confirm(BusinessActionContext context) {
        ...
        int i = freezeMapper.deleteById(context.getXid());
        return i==1;
    }

    /**
     * 回滚阶段 cancel+判断是否回滚+业务幂等处理
     * @param context
     * @return
     */
    @Override
...
改造AccountController中的业务接口为AccountTCCService

![image-20211203163322878](assets/image-20211203163322878.png)
