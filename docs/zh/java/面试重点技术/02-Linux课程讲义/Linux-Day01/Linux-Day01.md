...
<a-image src="assets/image-20210808234042174.png" alt="image-20210808234042174" style="zoom:80%;" /> 
...
基本的常用指令学会操作之后，我们可以在Linux服务器中安装项目运行常用的软件，比如： JDK、Tomcat、MySQL的安装及配置。

**4). 项目部署**

把基础的软件及环境安装配置好之后，我们就可以将我们开发的项目部署在服务器中进行运行并测试。

对于我们学习Linux操作系统时，Linux命令是基础，也是学习Linux系统的重点内容，虽然Linux的命令有很多，但是我们常用的命令并不多，大家在学习的时候，主要掌握这部分常用的命令就可以了。对于其他的命令，以后我们用到的时候知道怎么查询即可。
...
不同领域的主流操作系统，主要分为以下这么几类： 桌面操作系统、服务器操作系统、移动设备操作系统、嵌入式操作系统。这几个领域中代表性的操作系统是那些?
...
我们使用Linux操作系统，实际上选择的是Linux的发行版本，如下： 

| 发行版本  | Logo                                                         | 特点                             |
| --------- | ------------------------------------------------------------ | -------------------------------- |
| Ubuntu    | <a-image src="assets/image-20210809001838861.png" alt="image-20210809001838861" style="zoom:50%;" /> | 以桌面应用为主                   |
| RedHat    | <a-image src="assets/image-20210809001731378.png" alt="image-20210809001731378" style="zoom:50%;" /> | 应用最广泛、收费                 |
| CentOS    | <a-image src="assets/image-20210809001741238.png" alt="image-20210809001741238" style="zoom:50%;" /> | RedHat的社区版、免费             |
| openSUSE  | <a-image src="assets/image-20210809001750999.png" alt="image-20210809001750999" style="zoom:50%;" /> | 对个人完全免费、图形界面华丽     |
| Fedora    | <a-image src="assets/image-20210809001800676.png" alt="image-20210809001800676" style="zoom:50%;" /> | 功能完备、快速更新、免费         |
| 红旗Linux | <a-image src="assets/image-20210809001814942.png" alt="image-20210809001814942" style="zoom:50%;" /> | 北京中科红旗软件技术有限公司开发 |

除了上述罗列出来的发行版，还有很多Linux发行版，这里我们就不再一一列举了。
...
| 虚拟机安装 | 通过虚拟机软件安装               | 我们在学习阶段，没有自己的服务器，而我们又需要学Linux，就可以安装在虚拟机上 |
...
以上就是VMware在安装时的每一步操作，基本上就是点击 "下一步" 进行安装。
...
<a-image src="assets/image-20210809224601436.png" alt="image-20210809224601436" style="zoom:80%;" /> 



**2). 选择"典型"配置**

<a-image src="assets/image-20210809224640245.png" alt="image-20210809224640245"  /> 
...
<a-image src="assets/image-20210809230230390.png" alt="image-20210809230230390" style="zoom:80%;" />  



**9). 选择"Install CentOS7"**

<a-image src="assets/image-20210809230720673.png" alt="image-20210809230720673" style="zoom:80%;" /> 
...
<a-image src="assets/image-20210809231002444.png" alt="image-20210809231002444" style="zoom:80%;" /> 
...
服务器重启完成之后，我们可以通过linux的命令 `ip addr` 来查询Linux系统的IP地址，具体信息如下: 

<a-image src="assets/image-20210809233341574.png" alt="image-20210809233341574" style="zoom:80%;" /> 
...
> <font color='red'>备注: 在上述的操作中用到了Linux中的相关命令, 目前大家不用深究, 后面会详细讲解, 目前大家只需要把这个文件的配置项ONBOOT的值由no改为yes即可。</font>



2). 重新启动虚拟机

<a-image src="assets/image-20210809234617798.png" alt="image-20210809234617798" style="zoom:80%;" /> 

重启之后, 再次输入root的用户名密码，登录到Linux系统之后，可以通过指令 `ip addr` 来查看IP地址。

<a-image src="assets/image-20210809235056773.png" alt="image-20210809235056773"  /> 
...
Linux已经安装并且配置好了，接下来我们要来学习Linux的基本操作命令。而在学习之前，我们还需要做一件事情，由于我们企业开发时，Linux服务器一般都是在远程的机房部署的，我们要操作服务器，不会每次都跑到远程的机房里面操作，而是会直接通过SSH连接工具进行连接操作。
...
| SSH工具    | Logo                                                         | 说明                                    |
| ---------- | ------------------------------------------------------------ | --------------------------------------- |
| putty      | <a-image src="assets/image-20210809235613688.png" alt="image-20210809235613688" style="zoom:15%;" /> | 免费, 界面简单, 功能单一                |
| secureCRT  | <a-image src="assets/image-20210809235723109.png" alt="image-20210809235723109" style="zoom:20%;" /> | 收费, 功能强大                          |
| xshell     | <a-image src="assets/image-20210809235802640.png" alt="image-20210809235802640" style="zoom:50%;" /> | 收费版/免费版                           |
| finalshell | <a-image src="assets/image-20210809235841384.png" alt="image-20210809235841384" style="zoom: 67%;" /> | 免费, 功能强大, 界面效果好 (课程中采用) |
...
<a-image src="assets/image-20210810173004153.png" alt="image-20210810173004153" style="zoom:80%;" /> 



**2). 建立连接**

<a-image src="assets/image-20210810173138650.png" alt="image-20210810173138650" style="zoom:80%;" /> 
...
<a-image src="assets/image-20210810174831655.png" alt="image-20210810174831655" style="zoom: 80%;" /> 
...
在这一部分中介绍几个常用的命令，让大家快速感受一下Linux命令的操作方式。主要包括以下几个命令： 
...
<a-image src="assets/image-20210810232212430.png" alt="image-20210810232212430" style="zoom:80%;" /> 
...
说明: 
	...
	2). vim在进行文本编辑时共分为三种模式，分别是 **命令模式**（Command mode），**插入模式**（Insert mode）和**底行模式**（Last line mode）。这三种模式之间可以相互切换。我们在使用vim时一定要注意我们当前所处的是哪种模式。
...
​     <a-image src="assets/image-20210811194627474.png" alt="image-20210811194627474" style="zoom: 80%;" /> 
